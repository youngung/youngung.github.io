...
- 분석 및 해석 기술의 발달로 다량의 데이터들이 생성되고 있다. 재료공학에서 데이터 해석 및
  분석 능력의 중요성이 날로 높아지고 있다. 본 수업을 통해 학생들은 기초적인 데이터
  분석/해석 및 시각화(그래프)를 위해 필요한 다양한 툴을 익히고, 재료공학적 문제에 활용하는
  경험을 하게된다.
...
- 재료공학 전공자에게 필요한 데이터 분석, IO (input/output), 해석 및 시각화 (그래프)
 ...
 재료금속과 **전공 선택**교과이나, 이후 개설될 **수치해석**, **이동현상** 등의 교과과정의
 이해를 위해서 선수강하길 권한다.
...
- [구글](https://google.com) 계정을 활용해서
[Google Colab](https://colab.google)와
[Google Drive](https://drive.google.com/)를 함께 활용해서 실습 가능하다. 인터넷
이 연결된 환경에서 웹브라우저를 활용해 실습 가능하나 추천하지는 않는다. 이 경우 초기 환경
세팅이 수월하겠으나, 인터넷 접속이 유지되어야 하고 계산 속도가 다소 느리거나, 파일 I/O가
번거로울 수 있다. 아마 구글 Drive 서비스에서 연동이 가능할 듯하다. 하지만 추천 하지
않습니다.
...
1. 강의 자료는 [markdown](https://ko.wikipedia.org/wiki/마크다운) 파일로
   작성되어, [홈페이지에](https://youngung.github.io)내의 `lectures` 메뉴
   에 올려져 있으며 상시로 업데이트가 될 수 있다.
2. 원활한 수업 진행을 위해 수강생은 인터넷이 연결된 노트북을 가지고 오거나, 미리 수업 자료를
   출력해와야 한다 - 수업에 필수적인 준비가 안된 학생은 수업 태도/참여 점수 반영한다.
3. 수업 시간에 필요한 개념에 대해 설명하고, 교수자가 필요한 시연을 보인다. 이후 수강생들과
   함께 차근차근 실습하거나, 혹은 학생들이 **스스로** 그리고 **혼자서** 실습해봐야 한다
   :pray:
...
5. 때에 따라서, 기존에 입력한 코드가 이미 실행되어 있어야만 제대로 작동하는 경우가 있으니,
   ...
   놓친 부분을 마킹한 후, 강의 이후 스스로 찾아서 살펴보길 권한다.
...
- 본 수업은 재료과학/공학에서 활용되는 다양한 데이터 분석을 경험하고 활용하기 위해 필요한 기초
  적 도구(tool)의 활용을 익히는 것이 목표이기 때문에, 간단한 **코딩**(coding)이
  필요하다.
...
- 코딩을 위해서 필요한 적절한 환경이 갖춰진 컴퓨터:computer:가 없이 코딩을 배우는 건 매우
  ...
  친구/가족에게 노트북을 빌리는 것도 가능하나, 학교에서 대여도 한다 -
  [링크](https://chains.changwon.ac.kr/nonstop/lend/sub.php?group_code=e0000010&subgroup_code=es000043): 마지막으로 연결해본 날짜: 2026-06-05
...
- 수업 시간 예제 반드시 스스로 해보기
  - 스스로 공부할 때, 혹은 예제를 풀 때 [ChatGPT](https://chatgpt.com) 혹은
    [Gemini](https://gemini.google.com/app)에 도움을 받는 건 좋으나,
    단순 질문과 답을 구하기 보다 스스로의 이해를 돕기 위해 활용하길 바란다. 예를 들어 혼자
    해결하려 우선 노력해보고, 그 다음 정답을 찾아보는 방식으로 활용하기.
  - 예제를 일일이 다 쳐보기. 네모 박스 코드를 일일이 다 정확히 타자로 쳐보기 바란다.
    단순히 copy&paste 하기 보다 스스로 입력하고 그 결과를 이해하려는 노력 필요하다.
  - 결과를 스스로 살펴봐야 하고 이해하도록 노력해보기
  - 예제를 바꿔서 적용하고 그 변화를 살펴봐서 코드의 구조나 각 기능의 역할을 이해해야한다.
...
- 중간/기말 평가
  - 꾸준한 **출석**이 매우 중요한 교과목(20%)이며, 교과목 특성상 반드시 스스로 예제를
  풀이할 수 있어야 한다.
  - 단순 출석을 넘어서 수업에 적극적으로 참여하는 태도가 필요하다 (태도 점수 20%).
  - 중요한 원리를 이해하기 위해서는 설명을 읽는 것에 그치지 않고 직접 실습
    (타이핑, debugging, 수정 및 기능 추가 실습 등)을 해야 한다.
  ...
  - 자기가 필요한 프로젝트를 하나 설정한 다음에 해보면 더 좋다.
...
- Vibe-coding 서비스를 포함한 다양한 AI 도구를 경험해보는 것은 유익하다.
- 교수자 또한 AI 도구를 활용하고 강의 자료를 준비한다.
- 단 하나의 원칙을 지켜야 한다. 바로 사람이 먼저, 그 다음 AI 도구가 따라야 한다는 것이다.
- 우리 수업에서 여러분들이 가져가야 할 것은 바로 기본적인 원리와 사용 방법을 이해하는 것이다. 그리고 AI 도구를 효과적으로 활용하기 위해서는 기본적인 원리와 사용 방법을 이해하고 있어야 한다.
